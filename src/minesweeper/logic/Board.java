...
public class Board extends GridPane implements Serializable
{
    ...
    public Board(Difficulty difficulty)
    {
        ...
        for(int i = 0; i < this.difficulty.x(); i++)
        {
            for(int j = 0; j < this.difficulty.y(); j++)
            {
                this.add(this.board[j][i], i, j);
            }
        }
    ...
    public void addLand()
    {
        IntStream.range(0, this.difficulty.y())
                .forEach(y -> IntStream.range(0, this.difficulty.x())
                        .forEach(x -> this.board[y][x] = new Land(x, y)));
    }

    public void addBombs(int xSafe, int ySafe)
    {
        ...
        for(int i = 0; i < this.difficulty.bombs();)
        {
            ...
            {
                this.getChildren().remove(this.board[y][x]);
